...
const Work = () => {
    ...
    // ✂️ Filter images based on activeFilter
    ...
    return (
        <div className="min-h-screen bg-black text-white">
            {/* Header Section */}
            <div className="px-4 py-12 md:py-16 lg:py-20 text-center">
            ...
            {/* Filter Buttons */}
            <div className="flex flex-wrap justify-center gap-3 md:gap-4 px-4 mb-8 md:mb-12 lg:mb-16">
                {filters.map((filter) => (
                    <button
                        key={filter}
                        onClick={() => setActiveFilter(filter)}
                        className={`px-6 md:px-8 lg:px-10 py-3 md:py-3.5 text-sm md:text-base font-medium tracking-wider transition-all duration-300 ${activeFilter === filter
            ...
            <div className="px-4 md:px-6 lg:px-8 pb-12 md:pb-16 lg:pb-20">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8 max-w-6xl mx-auto">
                    {filteredGallery.map((item) => (
                        <div key={item.id} className="relative overflow-hidden group cursor-pointer rounded-lg">
                            <img
                                src={item.src}
                                alt={item.alt}
