import React from "react";
import AOSWrapper from "./AOSWrapper";

const About = () => {
  return (
    <section className="relative flex flex-col items-center justify-center text-center py-13 bg-black text-white px-4 overflow-hidden">
      
      {/* Background Stripes */}
      <AOSWrapper animation="fade-in">
        <div
          className="absolute inset-0 opacity-10"
          style={{
            backgroundImage:
              "repeating-linear-gradient(135deg, #d4c23b 0px, #d4c23b 2px, transparent 2px, transparent 40px)",
          }}
        />
      </AOSWrapper>

      {/* Content */}
      <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center">

        <AOSWrapper animation="fade-down">
          <p className="text-yellow-400 uppercase tracking-widest text-sm md:text-base font-semibold mb-4">
            Special Offer
          </p>
        </AOSWrapper>

        <AOSWrapper animation="zoom-in-up" delay={200}>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight">
            STUDENT
          </h1>
        </AOSWrapper>

        <AOSWrapper animation="zoom-in-up" delay={400}>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight text-yellow-400 mb-4">
            DISCOUNT
          </h1>
        </AOSWrapper>

        <AOSWrapper animation="fade-up" delay={600}>
          <p className="text-lg md:text-xl font-semibold mb-2">
            <span className="inline-block w-10 h-0.5 bg-yellow-400 align-middle mr-2"></span>
            MONDAY - WEDNESDAY
            <span className="inline-block w-10 h-0.5 bg-yellow-400 align-middle ml-2"></span>
          </p>
        </AOSWrapper>

        <AOSWrapper animation="fade-up" delay={800}>
          <p className="text-gray-400 max-w-xl mx-auto mt-4 text-sm md:text-base font-semibold leading-relaxed">
            Show your valid student ID and get exclusive discounts on all
            services every Monday through Wednesday.
          </p>
        </AOSWrapper>

        <AOSWrapper animation="flip-up" delay={1000}>
          <a
            href="#contact"
            className="mt-8 bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-3 px-8 rounded-md text-sm md:text-base transition-all duration-300"
          >
            BOOK YOUR APPOINTMENT
          </a>
        </AOSWrapper>

      </div>
    </section>
  );
};

export default About;
